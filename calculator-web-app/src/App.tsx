...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center p-4 selection:bg-indigo-500/30">
      <div className="w-full max-w-lg bg-zinc-900 rounded-[2.5rem] shadow-2xl border border-zinc-800/50 overflow-hidden flex flex-col md:flex-row">
        
        {/* Sidebar Navigation */}
        <div className="w-full md:w-20 bg-zinc-900/50 border-b md:border-b-0 md:border-r border-zinc-800 p-4 flex md:flex-col justify-around md:justify-start gap-4">
        ...
        {/* Main Content */}
        <div className="flex-1 flex flex-col p-6 min-h-[600px]">
          
          {/* Display Area */}
          <div className="mb-6 px-4 py-8 bg-zinc-950/50 rounded-3xl border border-zinc-800/30 flex flex-col items-end justify-center min-h-[140px]">
            <AnimatePresence mode="wait">
              <motion.div 
                key={expression}
                initial={{ opacity: 0, y: -5 }}
                animate={{ opacity: 0.5, y: 0 }}
                className="text-sm font-mono text-zinc-400 mb-1 h-5"
              >
                {expression}
              </motion.div>
            </AnimatePresence>
            <motion.div 
              ...
              className="text-5xl font-mono font-light tracking-tighter truncate w-full text-right"
            >
              {display}
            </motion.div>
          </div>
          ...
          <div className="flex-1">
            ...
            {mode === 'graphing' && (
              <div className="flex flex-col gap-4 h-full">
                <div className="flex gap-2">
                  <div className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 flex items-center gap-2">
                    <span className="text-indigo-400 font-mono italic">f(x) =</span>
                    <input 
                      value={graphFunction}
                      onChange={(e) => setGraphFunction(e.target.value)}
                      className="bg-transparent outline-none w-full font-mono text-zinc-100"
                      placeholder="sin(x)"
                    />
                  </div>
                  <button onClick={drawGraph} className="bg-indigo-600 p-3 rounded-xl hover:bg-indigo-500 transition-colors">
                    <RotateCcw size={20} />
                  </button>
                </div>
                <div className="flex-1 bg-zinc-950 rounded-2xl border border-zinc-800 overflow-hidden relative">
                  ...
                  />
                  <div className="absolute bottom-2 right-2 text-[10px] font-mono text-zinc-600 bg-zinc-950/80 px-2 py-1 rounded">
                    GRID: 1 UNIT = 40PX
                  </div>
                </div>
            ...
            {mode === 'programmer' && (
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 gap-2 bg-zinc-950 p-4 rounded-2xl border border-zinc-800">
                  <div className={`flex justify-between items-center p-2 rounded-lg ${progBase === 16 ? 'bg-indigo-500/10 border border-indigo-500/30' : ''}`} onClick={() => setProgBase(16)}>
                    <span className="text-xs font-bold text-zinc-500 uppercase">HEX</span>
                    <span className="font-mono text-indigo-400">{parseInt(display || '0').toString(16).toUpperCase()}</span>
                  </div>
                  <div className={`flex justify-between items-center p-2 rounded-lg ${progBase === 10 ? 'bg-indigo-500/10 border border-indigo-500/30' : ''}`} onClick={() => setProgBase(10)}>
                    <span className="text-xs font-bold text-zinc-500 uppercase">DEC</span>
                    <span className="font-mono text-zinc-100">{parseInt(display || '0').toString(10)}</span>
                  </div>
                  <div className={`flex justify-between items-center p-2 rounded-lg ${progBase === 8 ? 'bg-indigo-500/10 border border-indigo-500/30' : ''}`} onClick={() => setProgBase(8)}>
                    <span className="text-xs font-bold text-zinc-500 uppercase">OCT</span>
                    <span className="font-mono text-zinc-100">{parseInt(display || '0').toString(8)}</span>
                  </div>
                  <div className={`flex justify-between items-center p-2 rounded-lg ${progBase === 2 ? 'bg-indigo-500/10 border border-indigo-500/30' : ''}`} onClick={() => setProgBase(2)}>
                    <span className="text-xs font-bold text-zinc-500 uppercase">BIN</span>
                    <span className="font-mono text-zinc-100 text-xs break-all text-right">{parseInt(display || '0').toString(2).padStart(8, '0')}</span>
                  </div>
                </div>
          ...
          <AnimatePresence>
            {isHistoryOpen && (
              <motion.div
                ...
                className="absolute inset-y-0 right-0 w-full md:w-64 bg-zinc-900 border-l border-zinc-800 p-6 z-50 shadow-2xl"
              >
                ...
                <div className="flex flex-col gap-4 overflow-y-auto max-h-[calc(100%-60px)]">
                  {history.length === 0 ? (
      ...
      {/* Footer Info */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 text-zinc-600 text-[10px] font-mono tracking-widest uppercase">
